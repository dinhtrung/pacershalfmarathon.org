...
if ( !function_exists( 'runcrew_core_theme_setup' ) ) {
	...
	function runcrew_core_theme_setup() {
		...
		if ( !is_admin() ) {
			...
			//add_filter('wp_nav_menu_objects', 			'runcrew_add_mainmenu_classes', 10, 2);
			...
			add_action('wp_footer',		 					'runcrew_core_frontend_scripts_inline');
			add_action('runcrew_action_add_scripts_inline','runcrew_core_add_scripts_inline');
		...
		add_action('wp_ajax_nopriv_frontend_editor_save', 'runcrew_callback_frontend_editor_save');

		// Frontend editor: Delete post
		add_action('wp_ajax_frontend_editor_delete', 	'runcrew_callback_frontend_editor_delete');
		add_action('wp_ajax_nopriv_frontend_editor_delete', 'runcrew_callback_frontend_editor_delete');
...
function runcrew_core_init_theme() {
	...
	// Load skin
	$skin = runcrew_esc(runcrew_get_custom_option('theme_skin'));
...
if ( !function_exists( 'runcrew_register_theme_sidebars' ) ) {
	function runcrew_register_theme_sidebars($sidebars=array()) {
		...
		runcrew_storage_set('registered_sidebars', $sidebars);
...
if ( !function_exists( 'runcrew_core_frontend_scripts' ) ) {
	function runcrew_core_frontend_scripts() {
		...
		runcrew_enqueue_script( 'runcrew-core-modernizr-script', runcrew_get_file_url('js/photostack/modernizr.min.js'), array(), null, false );
		...
		if (is_array($theme_fonts) && count($theme_fonts) > 0) {
			...
			foreach ($theme_fonts as $font=>$v) {
				if (isset($fonts[$font])) {
					...
					if (!empty($fonts[$font]['css'])) {
						$css = $fonts[$font]['css'];
						runcrew_enqueue_style( 'runcrew-font-'.str_replace(' ', '-', $font_name).'-style', $css, array(), null );
					} else {
			...
			if ($google_fonts)
				runcrew_enqueue_style( 'runcrew-font-google_fonts-style', runcrew_get_protocol() . '://fonts.googleapis.com/css?family=' . $google_fonts . '&subset=' . runcrew_get_theme_option('fonts_subset'), array(), null );
        }
		
		// Fontello styles must be loaded before main stylesheet
		runcrew_enqueue_style( 'runcrew-fontello-style',  runcrew_get_file_url('css/fontello/css/fontello.css'),  array(), null);
		//runcrew_enqueue_style( 'runcrew-fontello-animation-style', runcrew_get_file_url('css/fontello/css/animation.css'), array(), null);

		// Main stylesheet
		runcrew_enqueue_style( 'runcrew-main-style', get_stylesheet_uri(), array(), null );
		...
		if (runcrew_get_theme_option('css_animation')=='yes' && (runcrew_get_theme_option('animation_on_mobile')=='yes' || !wp_is_mobile()) && !runcrew_vc_is_frontend())
			runcrew_enqueue_style( 'runcrew-animation-style',	runcrew_get_file_url('css/core.animation.css'), array(), null );
		...
		if (runcrew_get_theme_option('responsive_layouts') == 'yes') {
			...
			runcrew_enqueue_style( 'runcrew-responsive-style', runcrew_get_file_url('css/responsive'.($suffix).'.css'), array(), null );
		...
		runcrew_enqueue_script( 'superfish', runcrew_get_file_url('js/superfish.js'), array('jquery'), null, true );
		if (runcrew_get_theme_option('menu_slider')=='yes') {
			runcrew_enqueue_script( 'runcrew-slidemenu-script', runcrew_get_file_url('js/jquery.slidemenu.js'), array('jquery'), null, true );
			//runcrew_enqueue_script( 'runcrew-jquery-easing-script', runcrew_get_file_url('js/jquery.easing.js'), array('jquery'), null, true );
		}

		if ( is_single() && runcrew_get_custom_option('show_reviews')=='yes' ) {
			runcrew_enqueue_script( 'runcrew-core-reviews-script', runcrew_get_file_url('js/core.reviews.js'), array('jquery'), null, true );
		}

		runcrew_enqueue_script( 'runcrew-core-utils-script',	runcrew_get_file_url('js/core.utils.js'), array('jquery'), null, true );
		runcrew_enqueue_script( 'runcrew-core-init-script',	runcrew_get_file_url('js/core.init.js'), array('jquery'), null, true );	
		runcrew_enqueue_script( 'runcrew-theme-init-script',	runcrew_get_file_url('js/theme.init.js'), array('jquery'), null, true );	
...
//			wp_deregister_script('wp-mediaelement');
		...
		if (runcrew_get_custom_option('show_video_bg') == 'yes' && runcrew_get_custom_option('video_bg_youtube_code') != '') {
			runcrew_enqueue_script( 'runcrew-video-bg-script', runcrew_get_file_url('js/jquery.tubular.1.0.js'), array('jquery'), null, true );
		}
		...
		if (is_singular() && !runcrew_storage_get('blog_streampage') && runcrew_get_custom_option('show_share')!='hide') {
			runcrew_enqueue_script( 'runcrew-social-share-script', runcrew_get_file_url('js/social/social-share.js'), array('jquery'), null, true );
		}
		...
		if (runcrew_get_theme_option('show_theme_customizer') == 'yes') {
			if (file_exists(runcrew_get_file_dir('core/core.customizer/front.customizer.css')))
				runcrew_enqueue_style(  'runcrew-customizer-style',  runcrew_get_file_url('core/core.customizer/front.customizer.css'), array(), null );
			if (file_exists(runcrew_get_file_dir('core/core.customizer/front.customizer.js')))
				runcrew_enqueue_script( 'runcrew-customizer-script', runcrew_get_file_url('core/core.customizer/front.customizer.js'), array(), null, true );	
		}
		
		//Debug utils
		if (runcrew_get_theme_option('debug_mode')=='yes') {
			runcrew_enqueue_script( 'runcrew-core-debug-script', runcrew_get_file_url('js/core.debug.js'), array(), null, true );
		}
...
if ( !function_exists( 'runcrew_enqueue_slider' ) ) {
	function runcrew_enqueue_slider($engine='all') {
		if ($engine=='all' || $engine=='swiper') {
			runcrew_enqueue_style(  'runcrew-swiperslider-style', 			runcrew_get_file_url('js/swiper/swiper.css'), array(), null );
			runcrew_enqueue_script( 'runcrew-swiperslider-script', 			runcrew_get_file_url('js/swiper/swiper.js'), array(), null, true );
			// jQuery version conflict with Revolution Slider
			//runcrew_enqueue_script( 'runcrew-swiperslider-script', 			runcrew_get_file_url('js/swiper/swiper.jquery.js'), array(), null, true );
		}
	}
}
...
if ( !function_exists( 'runcrew_enqueue_polaroid' ) ) {
	function runcrew_enqueue_polaroid() {
		runcrew_enqueue_style(  'runcrew-polaroid-style', 	runcrew_get_file_url('js/photostack/component.css'), array(), null );
		runcrew_enqueue_script( 'runcrew-classie-script',		runcrew_get_file_url('js/photostack/classie.js'), array(), null, true );
		runcrew_enqueue_script( 'runcrew-polaroid-script',	runcrew_get_file_url('js/photostack/photostack.js'), array(), null, true );
	}
}
...
if ( !function_exists( 'runcrew_enqueue_messages' ) ) {
	function runcrew_enqueue_messages() {
		runcrew_enqueue_style(  'runcrew-messages-style',		runcrew_get_file_url('js/core.messages/core.messages.css'), array(), null );
		runcrew_enqueue_script( 'runcrew-messages-script',	runcrew_get_file_url('js/core.messages/core.messages.js'),  array('jquery'), null, true );
	}
}
...
if ( !function_exists( 'runcrew_enqueue_portfolio' ) ) {
	function runcrew_enqueue_portfolio($hover='') {
		runcrew_enqueue_style( 'runcrew-portfolio-style',  runcrew_get_file_url('css/core.portfolio.css'), array(), null );
		if (runcrew_strpos($hover, 'effect_dir')!==false)
			runcrew_enqueue_script( 'hoverdir', runcrew_get_file_url('js/hover/jquery.hoverdir.js'), array(), null, true );
	}
}
...
if ( !function_exists( 'runcrew_enqueue_diagram' ) ) {
	function runcrew_enqueue_diagram($type='all') {
		if ($type=='all' || $type=='pie') runcrew_enqueue_script( 'runcrew-diagram-chart-script',	runcrew_get_file_url('js/diagram/chart.min.js'), array(), null, true );
		if ($type=='all' || $type=='arc') runcrew_enqueue_script( 'runcrew-diagram-raphael-script',	runcrew_get_file_url('js/diagram/diagram.raphael.min.js'), array(), 'no-compose', true );
	}
}
...
if ( !function_exists( 'runcrew_enqueue_popup' ) ) {
	function runcrew_enqueue_popup($engine='') {
		if ($engine=='pretty' || (empty($engine) && runcrew_get_theme_option('popup_engine')=='pretty')) {
			runcrew_enqueue_style(  'runcrew-prettyphoto-style',	runcrew_get_file_url('js/prettyphoto/css/prettyPhoto.css'), array(), null );
			runcrew_enqueue_script( 'runcrew-prettyphoto-script',	runcrew_get_file_url('js/prettyphoto/jquery.prettyPhoto.min.js'), array('jquery'), 'no-compose', true );
		} else if ($engine=='magnific' || (empty($engine) && runcrew_get_theme_option('popup_engine')=='magnific')) {
			runcrew_enqueue_style(  'runcrew-magnific-style',	runcrew_get_file_url('js/magnific/magnific-popup.css'), array(), null );
			runcrew_enqueue_script( 'runcrew-magnific-script',runcrew_get_file_url('js/magnific/jquery.magnific-popup.min.js'), array('jquery'), '', true );
...
if ( !function_exists( 'runcrew_core_frontend_scripts_inline' ) ) {
	...
	function runcrew_core_frontend_scripts_inline() {
		add_filter('style_loader_tag', 'runcrew_core_add_property_to_link', 10, 3);
		do_action('runcrew_action_add_scripts_inline');
	}
}
...
if (!function_exists('runcrew_core_add_scripts_inline')) {
	function runcrew_core_add_scripts_inline() {
		...
		echo "<script type=\"text/javascript\">"
			...
			// AJAX parameters
			. "RUNCREW_STORAGE['ajax_url']			 = '" . esc_url(admin_url('admin-ajax.php')) . "';"
			. "RUNCREW_STORAGE['ajax_nonce']		 = '" . esc_attr(wp_create_nonce(admin_url('admin-ajax.php'))) . "';"
			
			// Site base url
			. "RUNCREW_STORAGE['site_url']			= '" . get_site_url() . "';"
			
			// VC frontend edit mode
			. "RUNCREW_STORAGE['vc_edit_mode']		= " . (function_exists('runcrew_vc_is_frontend') && runcrew_vc_is_frontend() ? 'true' : 'false') . ";"
			
			// Theme base font
			. "RUNCREW_STORAGE['theme_font']		= '" . runcrew_get_custom_font_settings('p', 'font-family') . "';"
			
			// Theme skin
			. "RUNCREW_STORAGE['theme_skin']			= '" . esc_attr(runcrew_get_custom_option('theme_skin')) . "';"
			...
			// Slider height
			. "RUNCREW_STORAGE['slider_height']	= " . max(100, runcrew_get_custom_option('slider_height')) . ";"
			...
			// User logged in
			. "RUNCREW_STORAGE['user_logged_in']	= " . (is_user_logged_in() ? 'true' : 'false') . ";"
			
			// Show table of content for the current page
			. "RUNCREW_STORAGE['toc_menu']		= '" . esc_attr(runcrew_get_custom_option('menu_toc')) . "';"
			. "RUNCREW_STORAGE['toc_menu_home']	= " . (runcrew_get_custom_option('menu_toc')!='hide' && runcrew_get_custom_option('menu_toc_home')=='yes' ? 'true' : 'false') . ";"
			. "RUNCREW_STORAGE['toc_menu_top']	= " . (runcrew_get_custom_option('menu_toc')!='hide' && runcrew_get_custom_option('menu_toc_top')=='yes' ? 'true' : 'false') . ";"
			
			// Fix main menu
			. "RUNCREW_STORAGE['menu_fixed']		= " . (runcrew_get_theme_option('menu_attachment')=='fixed' ? 'true' : 'false') . ";"
			
			// Use responsive version for main menu
			. "RUNCREW_STORAGE['menu_mobile']	= " . (runcrew_get_theme_option('responsive_layouts') == 'yes' ? max(0, (int) runcrew_get_theme_option('menu_mobile')) : 0) . ";"
			. "RUNCREW_STORAGE['menu_slider']     = " . (runcrew_get_theme_option('menu_slider')=='yes' ? 'true' : 'false') . ";"
			
			// Menu cache is used
			. "RUNCREW_STORAGE['menu_cache']	= " . (runcrew_get_theme_option('use_menu_cache')=='yes' ? 'true' : 'false') . ";"

			// Right panel demo timer
			. "RUNCREW_STORAGE['demo_time']		= " . (runcrew_get_theme_option('show_theme_customizer')=='yes' ? max(0, (int) runcrew_get_theme_option('customizer_demo')) : 0) . ";"

			// Video and Audio tag wrapper
			. "RUNCREW_STORAGE['media_elements_enabled'] = " . (runcrew_get_theme_option('use_mediaelement')=='yes' ? 'true' : 'false') . ";"
			
			// Use AJAX search
			. "RUNCREW_STORAGE['ajax_search_enabled'] 	= " . (runcrew_get_theme_option('use_ajax_search')=='yes' ? 'true' : 'false') . ";"
			. "RUNCREW_STORAGE['ajax_search_min_length']	= " . min(3, runcrew_get_theme_option('ajax_search_min_length')) . ";"
			. "RUNCREW_STORAGE['ajax_search_delay']		= " . min(200, max(1000, runcrew_get_theme_option('ajax_search_delay'))) . ";"

			// Use CSS animation
			. "RUNCREW_STORAGE['css_animation']      = " . (runcrew_get_theme_option('css_animation')=='yes' ? 'true' : 'false') . ";"
			. "RUNCREW_STORAGE['menu_animation_in']  = '" . esc_attr(runcrew_get_theme_option('menu_animation_in')) . "';"
			. "RUNCREW_STORAGE['menu_animation_out'] = '" . esc_attr(runcrew_get_theme_option('menu_animation_out')) . "';"

			// Popup windows engine
			. "RUNCREW_STORAGE['popup_engine']	= '" . esc_attr(runcrew_get_theme_option('popup_engine')) . "';"

			// E-mail mask
			. "RUNCREW_STORAGE['email_mask']		= '^([a-zA-Z0-9_\\-]+\\.)*[a-zA-Z0-9_\\-]+@[a-z0-9_\\-]+(\\.[a-z0-9_\\-]+)*\\.[a-z]{2,6}$';"
			
			// Messages max length
			. "RUNCREW_STORAGE['contacts_maxlength']	= " . intval(runcrew_get_theme_option('message_maxlength_contacts')) . ";"
			. "RUNCREW_STORAGE['comments_maxlength']	= " . intval(runcrew_get_theme_option('message_maxlength_comments')) . ";"

			// Remember visitors settings
			. "RUNCREW_STORAGE['remember_visitors_settings']	= " . (runcrew_get_theme_option('remember_visitors_settings')=='yes' ? 'true' : 'false') . ";"

			// Internal vars - do not change it!
			// Flag for review mechanism
			. "RUNCREW_STORAGE['admin_mode']			= false;"
			// Max scale factor for the portfolio and other isotope elements before relayout
			. "RUNCREW_STORAGE['isotope_resize_delta']	= 0.3;"
			// jQuery object for the message box in the form
			. "RUNCREW_STORAGE['error_message_box']	= null;"
			// Waiting for the viewmore results
			. "RUNCREW_STORAGE['viewmore_busy']		= false;"
			. "RUNCREW_STORAGE['video_resize_inited']	= false;"
			. "RUNCREW_STORAGE['top_panel_height']		= 0;"
...
if ( !function_exists( 'runcrew_enqueue_custom_styles' ) ) {
	function runcrew_enqueue_custom_styles() {
		// Custom stylesheet
		$custom_css = '';	//runcrew_get_custom_option('custom_stylesheet_url');
		runcrew_enqueue_style( 'runcrew-custom-style', $custom_css ? $custom_css : runcrew_get_file_url('css/custom-style.css'), array(), null );
