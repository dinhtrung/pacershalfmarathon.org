<?php
/**
 * RunCrew Framework: social networks
 *
 * @package	runcrew
 * @since	runcrew 1.0
 */
...
// Theme init
if (!function_exists('runcrew_socials_theme_setup')) {
	add_action( 'runcrew_action_before_init_theme', 'runcrew_socials_theme_setup' );
	function runcrew_socials_theme_setup() {

		if ( !is_admin() ) {
			// Add og:image meta tag for facebook
			add_action( 'wp_head', 					'runcrew_facebook_og_tags', 5 );
		}
...
runcrew_storage_set('share_links', array(
...
//	'facebook' =>		'http://www.facebook.com/sharer.php?s=100&p[url]={link}&p[title]={title}&p[summary]={descr}&p[images][0]={image}',
	...
	'postila' =>		'http://postila.ru/publish/?url={link}&agregator=runcrew',
...
if (!function_exists('runcrew_add_share_link')) {
	function runcrew_add_share_link($soc, $url='') {
		if (!is_array($soc)) $soc = array($soc => $url);
		runcrew_storage_set('share_links', array_merge( runcrew_storage_get('share_links'), $soc ) );
	}
}

// Return (and show) share social links
if (!function_exists('runcrew_show_share_links')) {
	function runcrew_show_share_links($args) {
		if ( runcrew_get_custom_option('show_share')=='hide' ) return '';
		$args = array_merge(array(
			...
			'counters' => runcrew_get_custom_option('show_share_counters')=='yes',	// show share counters
			'direction' => runcrew_get_custom_option('show_share'),				// share block direction
			'caption' => runcrew_get_custom_option('share_caption'),				// share block caption
			'share' => runcrew_get_theme_option('share_buttons'),					// list of allowed socials
		...
		if (is_array($args['share']) && count($args['share']) > 0) {
			foreach ($args['share'] as $soc) {
				...
				$url = $soc['url'];
				if (empty($url)) $url = runcrew_storage_get_array('share_links', $sn);
				...
				$output .= '<div class="sc_socials_item' . (!empty($args['popup']) ? ' social_item_popup' : '') . '">'
					...
					. ($args['type']=='drop' ? '<i>' . trim(runcrew_strtoproper($sn)) . '</i>' : '')
					. '</div>';
			}
		}
		$output .= '</div>';
...
if (!function_exists('runcrew_prepare_socials')) {
	function runcrew_prepare_socials($show_name=false, $list, $style='') {
		if (empty($style)) $style = runcrew_get_theme_setting('socials_type')=='images' ? 'bg' : 'icons';
		$output = '';
		$upload_info = wp_upload_dir();
		$upload_url = $upload_info['baseurl'];
        if (is_array($list) && count($list) > 0) {
			foreach ($list as $soc) {
				...
				$icon = $style=='icons' || runcrew_strpos($soc['icon'], $upload_url)!==false ? $soc['icon'] : runcrew_get_socials_url(basename($soc['icon']));
				if ($style == 'icons') {
					$parts = explode('-', $soc['icon'], 2);
					$sn = isset($parts[1]) ? $parts[1] : $parts[0];
				} else {
                ...
                $output .= '<div class="sc_socials_item">'
						...
						. ($style=='icons' 
							...
							: ($style=='images' 
								...
								: '<span class="sc_socials_hover" style="background-image: url('.esc_url($icon).');"></span>'))
						. '</a>'
						. ($show_name ? '<span class="name_social_link">' . esc_attr($sn) . '</span>' : '' )
						. '</div>';
			}
		}
		return $output;
...
if (!function_exists('runcrew_get_twitter_data')) {
	function runcrew_get_twitter_data($cfg) {
...
if (!function_exists('runcrew_prepare_twitter_text')) {
	function runcrew_prepare_twitter_text($tweet) {
		$text = $tweet['text'];
...
if (!function_exists('runcrew_get_twitter_followers')) {
	function runcrew_get_twitter_followers($cfg) {
		$data = runcrew_get_twitter_data($cfg); 
...
if (!function_exists('runcrew_get_facebook_likes')) {
	function runcrew_get_facebook_likes($account) {
		$fb = get_transient("facebooklikes");
		if ($fb !== false) return $fb;
		$fb = '?';
		$url = esc_url(runcrew_get_protocol().'http://graph.facebook.com/'.($account));
		$headers = get_headers($url);
		if (runcrew_strpos($headers[0], '200')) {
			$json = runcrew_fgc($url);
...
if (!function_exists('runcrew_facebook_og_tags')) {
	function runcrew_facebook_og_tags() {
		global $post;
		if ( !is_singular() || runcrew_storage_get('blog_streampage')) return;
...
if (!function_exists('runcrew_get_feedburner_counter')) {
	function runcrew_get_feedburner_counter($account) {
		$rss = get_transient("feedburnercounter");
		if ($rss !== false) return $rss;
		$rss = '?';
		$url = esc_url(runcrew_get_protocol().'://feedburner.google.com/api/awareness/1.0/GetFeedData?uri='.($account));
		$headers = get_headers($url);
		if (runcrew_strpos($headers[0], '200')) {
			$xml = runcrew_fgc($url);
