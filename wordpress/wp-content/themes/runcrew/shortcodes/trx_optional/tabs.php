...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('runcrew_sc_tabs_theme_setup')) {
	add_action( 'runcrew_action_before_init_theme', 'runcrew_sc_tabs_theme_setup' );
	function runcrew_sc_tabs_theme_setup() {
		add_action('runcrew_action_shortcodes_list', 		'runcrew_sc_tabs_reg_shortcodes');
		if (function_exists('runcrew_exists_visual_composer') && runcrew_exists_visual_composer())
			add_action('runcrew_action_shortcodes_list_vc','runcrew_sc_tabs_reg_shortcodes_vc');
	}
}
...
if (!function_exists('runcrew_sc_tabs')) {	
	function runcrew_sc_tabs($atts, $content = null) {
		...
		runcrew_storage_set('sc_tab_data', array(
			'counter'=> 0,
            'scroll' => $scroll,
		...
		$sc_tab_titles = runcrew_storage_get_array('sc_tab_data', 'titles');
		...
		runcrew_enqueue_script('jquery-effects-fade', false, array('jquery','jquery-effects-core'), null, true);
	
		$tabs_output .= $titles_output
			. '</ul>' 
			. ($content)
			.'</div>';
		return apply_filters('runcrew_shortcode_output', $tabs_output, 'trx_tabs', $atts, $content);
	}
	runcrew_require_shortcode("trx_tabs", "runcrew_sc_tabs");
}


if (!function_exists('runcrew_sc_tab')) {	
	function runcrew_sc_tab($atts, $content = null) {
		if (runcrew_in_shortcode_blogger()) return '';
		extract(runcrew_html_decode(shortcode_atts(array(
			// Individual params
		...
		runcrew_storage_inc_array('sc_tab_data', 'counter');
		$counter = runcrew_storage_get_array('sc_tab_data', 'counter');
		if (empty($id))
			$id = !empty($tab_id) ? $tab_id : runcrew_storage_get_array('sc_tab_data', 'id').'_'.intval($counter);
		$sc_tab_titles = runcrew_storage_get_array('sc_tab_data', 'titles');
		...
		runcrew_storage_set_array('sc_tab_data', 'titles', $sc_tab_titles);
		$output = '<div id="'.esc_attr($id).'"'
					.' class="sc_tabs_content' 
						...
						. ($css!='' ? ' style="'.esc_attr($css).'"' : '') 
						. '>' 
				. (runcrew_param_is_on(runcrew_storage_get_array('sc_tab_data', 'scroll')) 
					? '<div id="'.esc_attr($id).'_scroll" class="sc_scroll sc_scroll_vertical" style="height:'.(runcrew_storage_get_array('sc_tab_data', 'height') != '' ? runcrew_storage_get_array('sc_tab_data', 'height') : '200px').';"><div class="sc_scroll_wrapper swiper-wrapper"><div class="sc_scroll_slide swiper-slide">' 
					: '')
				...
				. (runcrew_param_is_on(runcrew_storage_get_array('sc_tab_data', 'scroll')) 
					...
					: '')
			. '</div>';
		return apply_filters('runcrew_shortcode_output', $output, 'trx_tab', $atts, $content);
	}
	runcrew_require_shortcode("trx_tab", "runcrew_sc_tab");
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'runcrew_sc_tabs_reg_shortcodes' ) ) {
	//add_action('runcrew_action_shortcodes_list', 'runcrew_sc_tabs_reg_shortcodes');
	function runcrew_sc_tabs_reg_shortcodes() {
	
		runcrew_sc_map("trx_tabs", array(
			"title" => esc_html__("Tabs", 'runcrew'),
			"desc" => wp_kses_data( __("Insert tabs in your page (post)", 'runcrew') ),
			"decorate" => true,
			"container" => false,
			"params" => array(
				"style" => array(
					"title" => esc_html__("Tabs style", 'runcrew'),
					"desc" => wp_kses_data( __("Select style for tabs items", 'runcrew') ),
					"value" => 1,
					"options" => runcrew_get_list_styles(1, 2),
					"type" => "radio"
				),
				"initial" => array(
					"title" => esc_html__("Initially opened tab", 'runcrew'),
					"desc" => wp_kses_data( __("Number of initially opened tab", 'runcrew') ),
					"divider" => true,
					"value" => 1,
					"min" => 0,
					"type" => "spinner"
				...
				"scroll" => array(
					...
					"desc" => wp_kses_data( __("Use scroller to show tab content (height parameter required)", 'runcrew') ),
					"divider" => true,
					"value" => "no",
					"type" => "switch",
			...
			"children" => array(
				"name" => "trx_tab",
				"title" => esc_html__("Tab", 'runcrew'),
				"desc" => wp_kses_data( __("Tab item", 'runcrew') ),
				"container" => true,
				"params" => array(
					"title" => array(
						"title" => esc_html__("Tab title", 'runcrew'),
						"desc" => wp_kses_data( __("Current tab title", 'runcrew') ),
						"value" => "",
						"type" => "text"
					),
					"_content_" => array(
						"title" => esc_html__("Tab content", 'runcrew'),
						"desc" => wp_kses_data( __("Current tab content", 'runcrew') ),
						"divider" => true,
						"rows" => 4,
						"value" => "",
						"type" => "textarea"
...
-------------------------------------------------------------------- */
if ( !function_exists( 'runcrew_sc_tabs_reg_shortcodes_vc' ) ) {
	//add_action('runcrew_action_shortcodes_list_vc', 'runcrew_sc_tabs_reg_shortcodes_vc');
	function runcrew_sc_tabs_reg_shortcodes_vc() {
	
		$tab_id_1 = 'sc_tab_'.time() . '_1_' . rand( 0, 100 );
		$tab_id_2 = 'sc_tab_'.time() . '_2_' . rand( 0, 100 );
		vc_map( array(
			"base" => "trx_tabs",
			"name" => esc_html__("Tabs", 'runcrew'),
			"desc" => wp_kses_data( __("Tabs", 'runcrew') ),
			...
			"params" => array(
				array(
					"param_name" => "style",
					"heading" => esc_html__("Tabs style", 'runcrew'),
					"desc" => wp_kses_data( __("Select style of tabs items", 'runcrew') ),
					"admin_label" => true,
					"class" => "",
				...
				array(
					"param_name" => "initial",
					"heading" => esc_html__("Initially opened tab", 'runcrew'),
					"desc" => wp_kses_data( __("Number of initially opened tab", 'runcrew') ),
					"class" => "",
					"value" => 1,
					"type" => "textfield"
				...
				array(
					"param_name" => "scroll",
					"heading" => esc_html__("Scroller", 'runcrew'),
					"desc" => wp_kses_data( __("Use scroller to show tab content (height parameter required)", 'runcrew') ),
					"class" => "",
			...
			'default_content' => '
				[trx_tab title="' . esc_html__( 'Tab 1', 'runcrew' ) . '" tab_id="'.esc_attr($tab_id_1).'"][/trx_tab]
				[trx_tab title="' . esc_html__( 'Tab 2', 'runcrew' ) . '" tab_id="'.esc_attr($tab_id_2).'"][/trx_tab]
			',
			"custom_markup" => '
		...
		vc_map( array(
			"base" => "trx_tab",
			"name" => esc_html__("Tab item", 'runcrew'),
			"desc" => wp_kses_data( __("Single tab item", 'runcrew') ),
			...
			"params" => array(
				array(
					"param_name" => "title",
					"heading" => esc_html__("Tab title", 'runcrew'),
					"desc" => wp_kses_data( __("Title for current tab", 'runcrew') ),
					"admin_label" => true,
					"class" => "",
					"value" => "",
					"type" => "textfield"
				...
				array(
					"param_name" => "tab_id",
					"heading" => esc_html__("Tab ID", 'runcrew'),
					"desc" => wp_kses_data( __("ID for current tab (required). Please, start it from letter.", 'runcrew') ),
					"admin_label" => true,
					"class" => "",
					"value" => "",
					"type" => "textfield"
		...
		class WPBakeryShortCode_Trx_Tabs extends RUNCREW_VC_ShortCodeTabs {}
		class WPBakeryShortCode_Trx_Tab extends RUNCREW_VC_ShortCodeTab {}
	}
}
?>