...
// Theme init
if (!function_exists('runcrew_widget_popular_posts_theme_setup')) {
	add_action( 'runcrew_action_before_init_theme', 'runcrew_widget_popular_posts_theme_setup', 1 );
	function runcrew_widget_popular_posts_theme_setup() {

		// Register shortcodes in the shortcodes list
		//add_action('runcrew_action_shortcodes_list',	'runcrew_widget_popular_posts_reg_shortcodes');
		if (function_exists('runcrew_exists_visual_composer') && runcrew_exists_visual_composer())
			add_action('runcrew_action_shortcodes_list_vc','runcrew_widget_popular_posts_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('runcrew_widget_popular_posts_load')) {
	add_action( 'widgets_init', 'runcrew_widget_popular_posts_load' );
	function runcrew_widget_popular_posts_load() {
		register_widget('runcrew_widget_popular_posts');
	}
}

// Widget Class
class runcrew_widget_popular_posts extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_popular_posts', 'description' => esc_html__('The most popular and most commented blog posts (extended)', 'runcrew'));
		parent::__construct( 'runcrew_widget_popular_posts', esc_html__('RunCrew - Most Popular & Commented Posts', 'runcrew'), $widget_ops );
	...
	function widget($args, $instance) {
		...
		$show_counters = isset($instance['show_counters']) && $instance['show_counters'] > 0 ? runcrew_get_theme_option('blog_counters') : '';
		...
		for ($i=0; $i<3; $i++) {
			...
			if ($i==0) {			// Most popular
				$args['meta_key'] = 'runcrew_post_views_count';
				$args['orderby'] = 'meta_value_num';
				$show_counters = $show_counters ? 'views' : '';
			} else if ($i==2) {		// Most liked
				$args['meta_key'] = 'runcrew_post_likes_count';
				$args['orderby'] = 'meta_value_num';
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_popular')); ?>"><?php esc_html_e('Most popular tab title:', 'runcrew'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_commented')); ?>"><?php esc_html_e('Most commented tab title:', 'runcrew'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_liked')); ?>"><?php esc_html_e('Most liked tab title:', 'runcrew'); ?></label>
...
*/
if ( !function_exists( 'runcrew_sc_widget_popular_posts' ) ) {
	function runcrew_sc_widget_popular_posts($atts, $content=null){	
		...
		extract($atts);
		$type = 'runcrew_widget_popular_posts';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_popular_posts' 
								. (runcrew_exists_visual_composer() ? ' vc_widget_popular_posts wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, runcrew_prepare_widgets_args(runcrew_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_popular_posts', 'widget_popular_posts') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('runcrew_shortcode_output', $output, 'trx_widget_popular_posts', $atts, $content);
	}
	runcrew_require_shortcode("trx_widget_popular_posts", "runcrew_sc_widget_popular_posts");
}


// Add [trx_widget_popular_posts] in the VC shortcodes list
if (!function_exists('runcrew_widget_popular_posts_reg_shortcodes_vc')) {
	//add_action('runcrew_action_shortcodes_list_vc','runcrew_widget_popular_posts_reg_shortcodes_vc');
	function runcrew_widget_popular_posts_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Popular Posts", 'runcrew'),
				"description" => wp_kses_data( __("Insert popular posts list with thumbs and post's meta", 'runcrew') ),
				...
				"params" => array(
					...
					array(
						...
						"heading" => esc_html__("Most popular tab title", 'runcrew'),
						"description" => wp_kses_data( __("Most popular tab title", 'runcrew') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						...
						"heading" => esc_html__("Most commented tab title", 'runcrew'),
						"description" => wp_kses_data( __("Most commented tab title", 'runcrew') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						...
						"heading" => esc_html__("Most liked tab title", 'runcrew'),
						"description" => wp_kses_data( __("Most liked tab title", 'runcrew') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
