...
// Theme init
if (!function_exists('runcrew_widget_recent_posts_theme_setup')) {
	add_action( 'runcrew_action_before_init_theme', 'runcrew_widget_recent_posts_theme_setup', 1 );
	function runcrew_widget_recent_posts_theme_setup() {

		// Register shortcodes in the shortcodes list
		//add_action('runcrew_action_shortcodes_list',	'runcrew_widget_recent_posts_reg_shortcodes');
		if (function_exists('runcrew_exists_visual_composer') && runcrew_exists_visual_composer())
			add_action('runcrew_action_shortcodes_list_vc','runcrew_widget_recent_posts_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('runcrew_widget_recent_posts_load')) {
	add_action( 'widgets_init', 'runcrew_widget_recent_posts_load' );
	function runcrew_widget_recent_posts_load() {
		register_widget('runcrew_widget_recent_posts');
	}
}

// Widget Class
class runcrew_widget_recent_posts extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_posts', 'description' => esc_html__('The recent blog posts (extended)', 'runcrew'));
		parent::__construct( 'runcrew_widget_recent_posts', esc_html__('RunCrew - Recent Posts', 'runcrew'), $widget_ops );
	...
	function form($instance) {
		...
		$counters = array_merge(array('hide' => esc_html__('Hide', 'runcrew')), runcrew_get_list_blog_counters(false));
		?>
...
*/
if ( !function_exists( 'runcrew_sc_widget_recent_posts' ) ) {
	function runcrew_sc_widget_recent_posts($atts, $content=null){	
		...
		extract($atts);
		$type = 'runcrew_widget_recent_posts';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_recent_posts' 
								. (runcrew_exists_visual_composer() ? ' vc_widget_recent_posts wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, runcrew_prepare_widgets_args(runcrew_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_posts', 'widget_recent_posts') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('runcrew_shortcode_output', $output, 'trx_widget_recent_posts', $atts, $content);
	}
	runcrew_require_shortcode("trx_widget_recent_posts", "runcrew_sc_widget_recent_posts");
}


// Add [trx_widget_recent_posts] in the VC shortcodes list
if (!function_exists('runcrew_widget_recent_posts_reg_shortcodes_vc')) {
	function runcrew_widget_recent_posts_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Recent Posts", 'runcrew'),
				"description" => wp_kses_data( __("Insert recent posts list with thumbs and post's meta", 'runcrew') ),
