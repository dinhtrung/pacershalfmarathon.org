...
function runcrew_skin_customizer(option, val) {
	...
	if (option == 'body_style') {
		...
		var params = RUNCREW_STORAGE['co_add_params']!=undefined ? RUNCREW_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#runcrew-customizer-styles-'+option).length > 0 ? jQuery('#runcrew-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="runcrew-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
