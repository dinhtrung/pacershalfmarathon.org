...
if ( !function_exists( 'runcrew_template_services_1_theme_setup' ) ) {
	add_action( 'runcrew_action_before_init_theme', 'runcrew_template_services_1_theme_setup', 1 );
	function runcrew_template_services_1_theme_setup() {
		runcrew_add_template(array(
			'layout' => 'services-1',
			'template' => 'services-1',
			'mode'   => 'services',
			'title'  => esc_html__('Services /Style 1/', 'runcrew'),
...
// Template output
if ( !function_exists( 'runcrew_template_services_1_output' ) ) {
	function runcrew_template_services_1_output($post_options, $post_data) {
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				if ($post_data['post_icon'] && $post_options['tag_type']=='icons') {
					$html = runcrew_do_shortcode('[trx_icon icon="'.esc_attr($post_data['post_icon']).'" shape="round"]');
                    echo trim($html);
				} else {
					?>
				...
				<div class="sc_services_item_content">
					...
					<div class="sc_services_item_description">
						...
						if ($post_data['post_protected']) {
							...
							if (!empty($post_data['post_link']) && $post_options['tag_type']=='icons') {
								?><a href="<?php echo esc_url($post_data['post_link']); ?>" class="sc_services_item_button icon-down"></a><?php
							}
                            if (!empty($post_data['post_link']) && $post_options['tag_type']!='icons' && !runcrew_param_is_off($post_options['readmore'])) {
