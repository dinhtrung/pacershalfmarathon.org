...
if ( !function_exists( 'runcrew_template_matches_2_theme_setup' ) ) {
	add_action( 'runcrew_action_before_init_theme', 'runcrew_template_matches_2_theme_setup', 1 );
	function runcrew_template_matches_2_theme_setup() {
		runcrew_add_template(array(
			'layout' => 'matches-2',
			'template' => 'matches-2',
			'mode'   => 'matches',
			/*'container_classes' => 'sc_slider_nopagination sc_slider_controls sc_slider_controls_bottom',*/
			'title'  => esc_html__('Matches /Style 2/', 'runcrew'),
...
// Template output
if ( !function_exists( 'runcrew_template_matches_2_output' ) ) {
	function runcrew_template_matches_2_output($post_options, $post_data) {
		$show_title = true;
