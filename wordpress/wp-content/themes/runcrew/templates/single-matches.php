...
if ( !function_exists( 'runcrew_template_single_matches_theme_setup' ) ) {
	add_action( 'runcrew_action_before_init_theme', 'runcrew_template_single_matches_theme_setup', 1 );
	function runcrew_template_single_matches_theme_setup() {
		runcrew_add_template(array(
			'layout' => 'single-matches',
			'mode'   => 'matches',
			'need_content' => true,
			'need_terms' => true,
			'title'  => esc_html__('Single matches', 'runcrew'),
...
// Template output
if ( !function_exists( 'runcrew_template_single_matches_output' ) ) {
	function runcrew_template_single_matches_output($post_options, $post_data) {
		...
		if (!$post_data['post_protected']) {
			...
			get_template_part(runcrew_get_file_slug('templates/trx_matches/_parts/match-info.php'));
		}
		...
		if ($post_data['post_protected']) { 
			...
			if ( runcrew_get_custom_option('show_post_tags') == 'yes' && !empty($post_data['post_terms'][$post_data['post_taxonomy_tags']]->terms_links)) {
				...
				<div class="post_info post_info_bottom">
					<span class="post_info_item post_info_tags"><?php esc_html_e('Tags:', 'runcrew'); ?> <?php echo join(', ', $post_data['post_terms'][$post_data['post_taxonomy_tags']]->terms_links); ?></span>
				</div>
				<?php 
			}
		} 
		...
		?>
			<div class="match_preview"><a href="<?php echo esc_url($match_preview); ?>"><?php echo esc_html__("Go to the announcement &#8594;", 'runcrew'); ?></a></div>
		<?php
		} 
		...
		if (!$sidebar_present) runcrew_close_wrapper();	// .post_item
