...
if ( !function_exists( 'runcrew_template_events_2_theme_setup' ) ) {
	add_action( 'runcrew_action_before_init_theme', 'runcrew_template_events_2_theme_setup', 1 );
	function runcrew_template_events_2_theme_setup() {
		runcrew_add_template(array(
			'layout' => 'events-2',
			'template' => 'events-2',
			'mode'   => 'events',
			'title'  => esc_html__('Events /Style 2/', 'runcrew')
		));
	}
}

// Template output
if ( !function_exists( 'runcrew_template_events_2_output' ) ) {
	function runcrew_template_events_2_output($post_options, $post_data) {
		$show_title = true;
