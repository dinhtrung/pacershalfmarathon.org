...
// Theme init
if (!function_exists('runcrew_widget_recent_reviews_theme_setup')) {
	add_action( 'runcrew_action_before_init_theme', 'runcrew_widget_recent_reviews_theme_setup', 1 );
	function runcrew_widget_recent_reviews_theme_setup() {

		// Register shortcodes in the shortcodes list
		//add_action('runcrew_action_shortcodes_list',	'runcrew_widget_recent_reviews_reg_shortcodes');
		if (function_exists('runcrew_exists_visual_composer') && runcrew_exists_visual_composer())
			add_action('runcrew_action_shortcodes_list_vc','runcrew_widget_recent_reviews_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('runcrew_widget_recent_reviews_load')) {
	add_action( 'widgets_init', 'runcrew_widget_recent_reviews_load' );
	function runcrew_widget_recent_reviews_load() {
		register_widget('runcrew_widget_recent_reviews');
	}
}

// Widget Class
class runcrew_widget_recent_reviews extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_reviews', 'description' => esc_html__('Recent reviews', 'runcrew'));
		parent::__construct( 'runcrew_widget_recent_reviews', esc_html__('RunCrew - Recent reviews', 'runcrew'), $widget_ops );
...
*/
if ( !function_exists( 'runcrew_sc_widget_recent_reviews' ) ) {
	function runcrew_sc_widget_recent_reviews($atts, $content=null){	
		...
		extract($atts);
		$type = 'runcrew_widget_recent_reviews';
		$output = '';
		...
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
						...
						. '">';
			ob_start();
			the_widget( $type, $atts, runcrew_prepare_widgets_args(runcrew_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_reviews', 'widget_recent_reviews') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('runcrew_shortcode_output', $output, 'trx_widget_recent_reviews', $atts, $content);
	}
	runcrew_require_shortcode("trx_widget_recent_reviews", "runcrew_sc_widget_recent_reviews");
}


// Add [trx_widget_recent_reviews] in the VC shortcodes list
if (!function_exists('runcrew_widget_recent_reviews_reg_shortcodes_vc')) {
	function runcrew_widget_recent_reviews_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Recent Reviews", 'runcrew'),
				"description" => wp_kses_data( __("Insert recent reviews list with thumbs and post's meta", 'runcrew') ),
