...
if ( !function_exists( 'runcrew_template_single_players_theme_setup' ) ) {
	add_action( 'runcrew_action_before_init_theme', 'runcrew_template_single_players_theme_setup', 1 );
	function runcrew_template_single_players_theme_setup() {
		runcrew_add_template(array(
			'layout' => 'single-players',
			'mode'   => 'players',
			'need_content' => true,
			'need_terms' => true,
			'title'  => esc_html__('Single Player', 'runcrew'),
...
// Template output
if ( !function_exists( 'runcrew_template_single_players_output' ) ) {
	function runcrew_template_single_players_output($post_options, $post_data) {
		$post_data['post_views']++;
		$show_title = runcrew_get_custom_option('show_post_title')=='yes';
		$title_tag = runcrew_get_custom_option('show_page_title')=='yes' ? 'h3' : 'h1';
		...
		if ($show_title && $post_options['location'] == 'center' && runcrew_get_custom_option('show_page_title')=='no') {
			?>
		...
		if (!$post_data['post_protected'] && (
			...
			(runcrew_get_custom_option('show_featured_image')=='yes' && $post_data['post_thumb'])	// && $post_data['post_format']!='gallery' && $post_data['post_format']!='image')
		)) {
			?>
		...
		runcrew_open_wrapper('<section class="post_content'.(!$post_data['post_protected'] && $post_data['post_edit_enable'] ? ' '.esc_attr('post_content_editor_present') : '').'" itemprop="articleBody">');
		...
		echo '<div class="player_info">'
				.(!empty($post_meta['player_country']) && $post_meta['player_country'] != 'inherit' ? '<span class="player_country">'. esc_html__( 'Country: ', 'runcrew' ) .''. runcrew_get_list_countries(false, $post_meta['player_country']) .'</span>' : '')
				.(!empty($post_meta['player_club']) && $post_meta['player_club'] != 'inherit' ? '<span class="player_club">'. esc_html__( 'Club: ', 'runcrew' ) .''. trim($post_meta['player_club']) .'</span>' : '')
				.(!empty($post_meta['player_age']) && $post_meta['player_age'] != 'inherit' && $post_meta['player_type'] == 'player' ? '<span class="player_age">'. esc_html__( 'Age: ', 'runcrew' ) .''. trim($post_meta['player_age']) .'</span>' : '')
			.'</div>';
			
		// Player socials
		if(!empty($post_meta['player_socials'])){ 
			$socials = $post_meta['player_socials'];
			echo trim(runcrew_do_shortcode('[trx_socials size="tiny" shape="round" socials="'.esc_attr($socials).'"][/trx_socials]'));
		}
