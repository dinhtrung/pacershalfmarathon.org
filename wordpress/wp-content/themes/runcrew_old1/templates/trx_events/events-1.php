...
if ( !function_exists( 'runcrew_template_events_1_theme_setup' ) ) {
	add_action( 'runcrew_action_before_init_theme', 'runcrew_template_events_1_theme_setup', 1 );
	function runcrew_template_events_1_theme_setup() {
		runcrew_add_template(array(
			'layout' => 'events-1',
			'template' => 'events-1',
			'mode'   => 'events',
			'title'  => esc_html__('Events /Style 1/', 'runcrew'),
...
// Template output
if ( !function_exists( 'runcrew_template_events_1_output' ) ) {
	function runcrew_template_events_1_output($post_options, $post_data) {
		$show_title = true;
							...
							if ($post_data['post_excerpt']) {
								echo in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status')) 
										...
										: '<p>'.trim(runcrew_strshort($post_data['post_excerpt'], isset($post_options['descr']) 
																										? $post_options['descr'] 
																										: runcrew_get_custom_option('post_excerpt_maxlength_masonry'))).'</p>';
							}
