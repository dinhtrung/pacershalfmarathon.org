...
if ( !function_exists( 'runcrew_template_single_standard_theme_setup' ) ) {
	add_action( 'runcrew_action_before_init_theme', 'runcrew_template_single_standard_theme_setup', 1 );
	function runcrew_template_single_standard_theme_setup() {
		runcrew_add_template(array(
			'layout' => 'single-standard',
			'mode'   => 'single',
			'need_content' => true,
			'need_terms' => true,
			'title'  => esc_html__('Single standard', 'runcrew'),
...
// Template output
if ( !function_exists( 'runcrew_template_single_standard_output' ) ) {
	function runcrew_template_single_standard_output($post_options, $post_data) {
		...
		$title_tag = runcrew_get_custom_option('show_page_title')=='yes' ? 'h3' : 'h2';
		...
		if (!$post_data['post_protected'] && runcrew_get_custom_option('show_post_info')=='yes') {
			$post_options['info_parts'] = array('snippets'=>false, 'author'=>false, 'terms' => false);
