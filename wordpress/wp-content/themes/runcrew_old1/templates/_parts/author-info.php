...
if (runcrew_get_custom_option("show_post_author") == 'yes') {
	$post_author_name = $post_author_socials = '';
	$show_post_author_socials = true;
	if ($post_data['post_type']=='post') {
		$post_author_title = esc_html__('About author:', 'runcrew');
		...
		$post_author_avatar = get_avatar($post_author_email, 138*$mult);
		$post_author_descr = runcrew_do_shortcode(nl2br(get_the_author_meta('description', $post_data['post_author_id'])));
		if ($show_post_author_socials) 
			$post_author_socials = runcrew_show_user_socials( array(
