...
if ( !function_exists( 'runcrew_template_masonry_theme_setup' ) ) {
	add_action( 'runcrew_action_before_init_theme', 'runcrew_template_masonry_theme_setup', 1 );
	function runcrew_template_masonry_theme_setup() {
		runcrew_add_template(array(
			'layout' => 'masonry_2',
			'template' => 'masonry',
			'mode'   => 'blog',
			'need_isotope' => true,
			'title'  => esc_html__('Masonry tile (different height) /2 columns/', 'runcrew'),
		...
		runcrew_add_template(array(
			...
			'mode'   => 'blog',
			'need_isotope' => true,
			'title'  => esc_html__('Masonry tile /3 columns/', 'runcrew'),
			'thumb_title'  => esc_html__('Medium image', 'runcrew'),
			'w'		 => 370,
			'h_crop' => 209,
			'h'      => null
		...
		runcrew_add_template(array(
			...
			'mode'   => 'blog',
			'need_isotope' => true,
			'title'  => esc_html__('Masonry tile /4 columns/', 'runcrew'),
			'thumb_title'  => esc_html__('Medium image', 'runcrew'),
			'w'		 => 370,
			'h_crop' => 209,
			'h'      => null
		...
		runcrew_add_template(array(
			...
			'mode'   => 'blog',
			'need_isotope' => true,
			'title'  => esc_html__('Classic tile (equal height) /2 columns/', 'runcrew'),
		...
		runcrew_add_template(array(
			...
			'mode'   => 'blog',
			'need_isotope' => true,
			'title'  => esc_html__('Classic tile /3 columns/', 'runcrew'),
            'thumb_title'  => esc_html__('Medium square image (crop)', 'runcrew'),
            'w'		 => 370,
            'h' 	 => 370
		));
		runcrew_add_template(array(
			...
			'mode'   => 'blog',
			'need_isotope' => true,
			'title'  => esc_html__('Classic tile /4 columns/', 'runcrew'),
            'thumb_title'  => esc_html__('Medium square image (crop)', 'runcrew'),
            'w'		 => 370,
            'h' 	 => 370
		));
		// Add template specific scripts
		add_action('runcrew_action_blog_scripts', 'runcrew_template_masonry_add_scripts');
	}
}

// Add template specific scripts
if (!function_exists('runcrew_template_masonry_add_scripts')) {
	//add_action('runcrew_action_blog_scripts', 'runcrew_template_masonry_add_scripts');
	function runcrew_template_masonry_add_scripts($style) {
		if (in_array(runcrew_substr($style, 0, 8), array('classic_', 'masonry_'))) {
...
// Template output
if ( !function_exists( 'runcrew_template_masonry_output' ) ) {
	function runcrew_template_masonry_output($post_options, $post_data) {
		...
		<div class="isotope_item isotope_item_<?php echo esc_attr($style); ?> isotope_item_<?php echo esc_attr($post_options['layout']); ?> isotope_column_<?php echo esc_attr($columns); ?>
			...
			<<?php echo trim($tag); ?> class="post_item post_item_<?php echo esc_attr($style); ?> post_item_<?php echo esc_attr($post_options['layout']); ?>
				...
				<div class="post_content isotope_item_content">
                    ...
                    if (!$post_data['post_protected'] && $post_options['info'] && !in_array($style, array('masonry', 'classic'))) {
					...
					<div class="post_descr">
						...
						if ($post_data['post_protected']) {
                            ...
                            if (!in_array($style, array('masonry', 'classic'))) {
                                if (!runcrew_param_is_off($post_options['readmore']) && !in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status'))) {
                                    echo trim(runcrew_sc_button(array('link'=>$post_data['post_link'], 'class'=>"post_readmore"), $post_options['readmore']));
                                }
                            }
                            if (!$post_data['post_protected'] && $post_options['info'] && in_array($style, array('masonry', 'classic'))) {
                                    $post_options['info_parts'] = array('counters'=>true, 'terms'=>false, 'author'=>false);
