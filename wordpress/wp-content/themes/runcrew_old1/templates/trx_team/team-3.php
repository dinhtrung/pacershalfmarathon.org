...
if ( !function_exists( 'runcrew_template_team_3_theme_setup' ) ) {
	add_action( 'runcrew_action_before_init_theme', 'runcrew_template_team_3_theme_setup', 1 );
	function runcrew_template_team_3_theme_setup() {
		runcrew_add_template(array(
			'layout' => 'team-3',
			'template' => 'team-3',
			'mode'   => 'team',
			/*'container_classes' => 'sc_slider_nopagination sc_slider_controls sc_slider_controls_side',*/
			'title'  => esc_html__('Team /Style 3/', 'runcrew'),
...
// Template output
if ( !function_exists( 'runcrew_template_team_3_output' ) ) {
	function runcrew_template_team_3_output($post_options, $post_data) {
		$show_title = true;
		$parts = explode('_', $post_options['layout']);
		$style = $parts[0];
        $team_title  = get_the_title($post_data['post_id']);
        ...
        ?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				<div class="sc_team_item_info">
					<h5 class="sc_team_item_title"><?php echo (!empty($post_options['link']) ? '<a href="'.esc_url($post_options['link']).'">' : '') . esc_attr($team_title) . (!empty($post_options['link']) ? '</a>' : ''); ?></h5>
