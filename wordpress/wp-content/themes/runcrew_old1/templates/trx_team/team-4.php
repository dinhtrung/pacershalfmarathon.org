...
if ( !function_exists( 'runcrew_template_team_4_theme_setup' ) ) {
	add_action( 'runcrew_action_before_init_theme', 'runcrew_template_team_4_theme_setup', 1 );
	function runcrew_template_team_4_theme_setup() {
		runcrew_add_template(array(
			'layout' => 'team-4',
			'template' => 'team-4',
			'mode'   => 'team',
			/*'container_classes' => 'sc_slider_nopagination sc_slider_controls sc_slider_controls_side',*/
			'title'  => esc_html__('Team /Style 4/', 'runcrew'),
			'thumb_title'  => esc_html__('Medium vertical image (crop)', 'runcrew'),
            'w' => 368,
            'h' => 424
		));
	}
...
// Template output
if ( !function_exists( 'runcrew_template_team_4_output' ) ) {
	function runcrew_template_team_4_output($post_options, $post_data) {
		$show_title = true;
				...
				<div class="sc_team_item_avatar"><?php echo trim($post_options['photo']); ?>
					<div class="sc_team_item_hover">
						<div class="sc_team_item_info">
							...
							<div class="sc_team_item_description"><?php echo trim(runcrew_strshort($post_data['post_excerpt'], isset($post_options['descr']) ? $post_options['descr'] : runcrew_get_custom_option('post_excerpt_maxlength_masonry'))); ?></div>
