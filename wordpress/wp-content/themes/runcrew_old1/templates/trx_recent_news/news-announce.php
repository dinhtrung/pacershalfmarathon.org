...
if ( !function_exists( 'runcrew_template_news_announce_theme_setup' ) ) {
	add_action( 'runcrew_action_before_init_theme', 'runcrew_template_news_announce_theme_setup', 1 );
	function runcrew_template_news_announce_theme_setup() {
		runcrew_add_template(array(
			'layout' => 'news-announce',
			'template' => 'news-announce',
			'mode'   => 'news',
			'title'  => esc_html__('Recent News /Style Announce/', 'runcrew')
		));

		// Add thumb sizes into list
		runcrew_add_thumb_sizes( array( 'thumb_slug' => 'full', 'add_image_size' => false, 'w' => 1170, 'h' => 659 ) );
		runcrew_add_thumb_sizes( array( 'thumb_slug' => 'big', 'add_image_size' => false, 'w' => 770, 'h' => 434 ) );
		runcrew_add_thumb_sizes( array( 'thumb_slug' => 'medium', 'add_image_size' => false, 'w' => 770, 'h' => 217 ) );
		runcrew_add_thumb_sizes( array( 'thumb_slug' => 'small', 'add_image_size' => false, 'w' => 370, 'h' => 209 ) );
	}
}

// Template output
if ( !function_exists( 'runcrew_template_news_announce_output' ) ) {
	function runcrew_template_news_announce_output($post_options, $post_data) {
		...
		$post_data['post_thumb'] = runcrew_get_resized_image_tag($post_data['post_attachment'], $thumb_sizes['w'], $post_data['post_type']=='product' && runcrew_get_theme_option('crop_product_thumb')=='no' ? null :  $thumb_sizes['h']);
