...
if ( !function_exists( 'runcrew_template_services_2_theme_setup' ) ) {
	add_action( 'runcrew_action_before_init_theme', 'runcrew_template_services_2_theme_setup', 1 );
	function runcrew_template_services_2_theme_setup() {
		runcrew_add_template(array(
			'layout' => 'services-2',
			'template' => 'services-2',
			'mode'   => 'services',
			'need_columns' => true,
			'title'  => esc_html__('Services /Style 2/', 'runcrew')
		));
	}
}

// Template output
if ( !function_exists( 'runcrew_template_services_2_output' ) ) {
	function runcrew_template_services_2_output($post_options, $post_data) {
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				if ($post_data['post_icon'] && $post_options['tag_type']=='icons') {
					...
					?>
					<div class="sc_services_item_featured post_featured" style="background-image: url(<?php echo esc_url($post_data['post_attachment']); ?>)">
					</div>
					<?php
				}
				?>
				<div class="sc_services_item_content">
					...
					<div class="sc_services_item_description">
						...
						if ($post_data['post_protected']) {
							...
							if (!empty($post_data['post_link']) && !runcrew_param_is_off($post_options['readmore'])) {
                                echo trim(runcrew_sc_button(array('link'=>$post_data['post_link']), trim($post_options['readmore'])));
							}
						}
						?>
					</div>
