...
if ( !function_exists( 'runcrew_template_services_4_theme_setup' ) ) {
	add_action( 'runcrew_action_before_init_theme', 'runcrew_template_services_4_theme_setup', 1 );
	function runcrew_template_services_4_theme_setup() {
		runcrew_add_template(array(
			'layout' => 'services-4',
			'template' => 'services-4',
			'mode'   => 'services',
			'need_columns' => true,
			'title'  => esc_html__('Services /Style 4/', 'runcrew')
		));
	}
}

// Template output
if ( !function_exists( 'runcrew_template_services_4_output' ) ) {
	function runcrew_template_services_4_output($post_options, $post_data) {
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				echo trim(runcrew_do_shortcode('[trx_icon icon="'.esc_attr($post_data['post_icon']).'" shape="round"]'));
