...
if ( !function_exists( 'runcrew_template_list_theme_setup' ) ) {
	add_action( 'runcrew_action_before_init_theme', 'runcrew_template_list_theme_setup', 1 );
	function runcrew_template_list_theme_setup() {
		runcrew_add_template(array(
			'layout' => 'list',
			'mode'   => 'blogger',
			'need_columns' => true,
			'title'  => esc_html__('Blogger layout: List', 'runcrew')
			));
	}
}

// Template output
if ( !function_exists( 'runcrew_template_list_output' ) ) {
	function runcrew_template_list_output($post_options, $post_data) {
