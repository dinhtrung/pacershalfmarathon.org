...
jQuery(document).ready(function(){
	...
	jQuery('.trx_importer_section').on('click', '.trx_buttons input[type="button"]', function() {
		...
		RUNCREW_STORAGE['importer_error_messages'] = '';
		RUNCREW_STORAGE['importer_ignore_errors'] = true;
		runcrew_importer_do_action(steps, 0);
	});
});

// Call specified action (step)
function runcrew_importer_do_action(steps, idx) {
	"use strict";
	...
	// AJAX query params
	var data = {
		ajax_nonce: RUNCREW_STORAGE['ajax_nonce'],
		action: 'runcrew_importer_start_import',
	...
	jQuery.post(RUNCREW_STORAGE['ajax_url'], data, function(response) {
		...
		if (rez.error === '' || RUNCREW_STORAGE['importer_ignore_errors']) {
			if (rez.error !== '') 
				RUNCREW_STORAGE['importer_error_messages'] += '<p class="error_message">' + rez.error + '</p>';
			...
			// Do next action
			if (idx < steps.length) {
				runcrew_importer_do_action(steps, idx);
			} else {
				if (RUNCREW_STORAGE['importer_error_messages']) {
					jQuery('#trx_importer_progress').removeClass('notice-info').addClass('notice-error').append('<h4>' + RUNCREW_STORAGE['importer_error_msg'] + '</h4>' + RUNCREW_STORAGE['importer_error_messages']);
				} else {
