...
jQuery(window).on('beforeunload', function() {
	"use strict";
	// Show preloader
//	jQuery('body.preloader>.outer_wrap,body.preloader>.body_wrap,body.preloader>.runcrew_profiler').css({opacity:0});	//.animate({opacity:0}, 'fast');
...
function runcrew_init_actions() {
	...
	// Hide preloader
//	jQuery('body.preloader>.outer_wrap,body.preloader>.body_wrap,body.preloader>.runcrew_profiler').animate({opacity:1}, 'slow');	//.css({opacity:1});
...
function runcrew_ready_actions() {
	...
	// Prepare menus
	if (RUNCREW_STORAGE['menu_cache']) runcrew_prepare_menus();
	...
	if (jQuery('.header_mobile').length > 0) {
		...
		jQuery('.header_mobile .menu_button').on('click', function(){
			...
			// Fix for Safari
			if (runcrew_browser_is_ios() && jQuery('body').hasClass('menu_mobile') && jQuery('div.side_wrap').hasClass('open')) {
	...
	jQuery('.woocommerce,.woocommerce-page').on('click', '.mode_buttons a', function(e) {
		...
		jQuery.cookie('runcrew_shop_mode', mode, {expires: 365, path: '/'});
	...
	jQuery('.popup_form.login_form').submit(function(e){
		"use strict";
		var rez = runcrew_login_validate(jQuery(this));
		if (!rez)
			e.preventDefault();
		return rez;
	});
	...
	jQuery('.popup_form.registration_form').submit(function(e){
		"use strict";
		var rez = runcrew_registration_validate(jQuery(this));
		if (!rez)
			e.preventDefault();
		return rez;
	});
	...
	jQuery("form#commentform").submit(function(e) {
		"use strict";
		var rez = runcrew_comments_validate(jQuery(this));
		if (!rez)
			e.preventDefault();
		return rez;
	});
		...
		if (scroll_offset + jQuery(window).height() > RUNCREW_STORAGE['side_panel_height'] + 100) {
			if (jQuery('.sidebar_outer').css('position')!=='fixed') {
				jQuery('.sidebar_outer').css({
					//'top': (scroll_offset + jQuery(window).height() - RUNCREW_STORAGE['side_panel_height'] - 100) + 'px'
					'position': 'fixed',
...
function runcrew_prepare_menus() {
	"use strict";
	var menus = [
		jQuery('ul#menu_main'),
		jQuery('ul#menu_user'),
...
// Comments form
function runcrew_comments_validate(form) {
	"use strict";
	form.find('input').removeClass('error_fields_class');
	var rules = {
		error_message_text: RUNCREW_STORAGE['strings']['error_global'],	// Global error message text (if don't write in checked field)
		...
		rules: [
			{
				...
				max_length: { value: RUNCREW_STORAGE['comments_maxlength'], message: RUNCREW_STORAGE['strings']['text_long']}
			}
		]
	};
	...
	var error = runcrew_form_validate(form, rules);
	return !error;
}


// Login form
function runcrew_login_validate(form) {
	"use strict";
	form.find('input').removeClass('error_fields_class');
	var error = runcrew_form_validate(form, {
		...
		rules: [
			{
				field: "log",
				min_length: { value: 1, message: RUNCREW_STORAGE['strings']['login_empty'] },
				max_length: { value: 60, message: RUNCREW_STORAGE['strings']['login_long'] }
			},
			{
				field: "pwd",
				min_length: { value: 4, message: RUNCREW_STORAGE['strings']['password_empty'] },
				max_length: { value: 30, message: RUNCREW_STORAGE['strings']['password_long'] }
			}
		]
	});
	if (!error) {
		jQuery.post(RUNCREW_STORAGE['ajax_url'], {
			...
			if (rez.error === '') {
				result_box.addClass('sc_infobox sc_infobox_style_success').html(RUNCREW_STORAGE['strings']['login_success']);
				setTimeout(function() { 
					location.reload(); 
					}, 3000);
			} else {
				result_box.addClass('sc_infobox sc_infobox_style_error').html(RUNCREW_STORAGE['strings']['login_failed'] + '<br>' + rez.error);
			}
...
// Registration form 
function runcrew_registration_validate(form) {
	"use strict";
	form.find('input').removeClass('error_fields_class');
	var error = runcrew_form_validate(form, {
		error_message_show: true,
		...
		rules: [
			...
			},
			{
				...
				equal_to: { value: 'registration_pwd', message: RUNCREW_STORAGE['strings']['password_not_equal'] }
			}
		]
	});
	if (!error) {
		jQuery.post(RUNCREW_STORAGE['ajax_url'], {
			...
			result_box.toggleClass('sc_infobox_style_error', false).toggleClass('sc_infobox_style_success', false);
			if (rez.error === '') {
				result_box.addClass('sc_infobox sc_infobox_style_success').html(RUNCREW_STORAGE['strings']['registration_success']);
				setTimeout(function() { 
					jQuery('.popup_login_link').trigger('click'); 
					}, 3000);
			} else {
				result_box.addClass('sc_infobox sc_infobox_style_error').html(RUNCREW_STORAGE['strings']['registration_failed'] + ' ' + rez.error);
			}
			result_box.fadeIn().delay(3000).fadeOut();
		});
	}
